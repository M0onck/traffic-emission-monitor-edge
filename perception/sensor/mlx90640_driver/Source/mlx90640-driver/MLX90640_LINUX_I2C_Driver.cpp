...
void MLX90640_I2CInit()
{
    if (!i2c_fd) {
        ...
        if (i2c_fd >= 0) {
            // 设置超时时间，单位为 10 毫秒。50 代表 500 毫秒。
            // 只要硬件挂起超过 500ms，内核就会主动打断并返回错误，而不会永久死锁
            if (ioctl(i2c_fd, I2C_TIMEOUT, 50) < 0) {
                printf("[MLX90640 Driver] Warning: Failed to set I2C_TIMEOUT\n");
            }
            
            // 开启内核底层的自动重试机制（最高重试 3 次）。
            if (ioctl(i2c_fd, I2C_RETRIES, 3) < 0) {
                printf("[MLX90640 Driver] Warning: Failed to set I2C_RETRIES\n");
            }
        } else {
            printf("[MLX90640 Driver] Error: Failed to open I2C bus %s\n", i2c_device);
        }
    }
}
